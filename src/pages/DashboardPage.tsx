...
import {
  TrendingUp, Users, DollarSign, Calendar, Plus, Trash2,
  Copy, Check, Sparkles,
} from 'lucide-react';
...
function CopywritingPanel() {
  ...
  const handleGenerate = () => {
    setResult(generateCopy(platform, tone, topic));
    toast.success('文案已生成');
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <Sparkles className="h-4 w-4" /> AI 营销文案生成
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div>
            <Label>主题</Label>
            <Input value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="如：日系写真" />
          </div>
        </div>
        <Button onClick={handleGenerate}><Sparkles className="mr-1.5 h-4 w-4" /> 生成文案</Button>
        {result && (
          <div className="relative rounded-lg border bg-muted/30 p-4">
            <p className="whitespace-pre-wrap pr-8 text-sm">{result}</p>
...
export default function DashboardPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const pendingBalance = orders.reduce((s, o) => s + o.balance, 0);
    const monthOrders = orders.length;
    const activeCustomers = customers.length;
    return { totalRevenue, pendingBalance, monthOrders, activeCustomers };
  }, [orders, customers]);
  ...
  useEffect(() => {
    ...
    revenueChart.setOption({
      ...
      series: [{
        type: 'line', smooth: true, data: [8600, 12300, 9800, 15600, 18900, 22400],
    ...
    const sourceCount = customers.reduce<Record<string, number>>((acc, c) => {
    ...
    sourceChart.setOption({
      ...
      series: [{
        ...
        data: Object.entries(sourceCount).map(([name, value], i) => ({
    ...
    statusChart.setOption({
      ...
      yAxis: {
        type: 'category',
        data: Object.keys(statusCount).map((s) => ORDER_STATUS_MAP[s as OrderStatus].label),
      },
      series: [{
        type: 'bar',
        data: Object.values(statusCount).map((v, i) => ({
          value: v, itemStyle: { color: CHART_COLORS[i % CHART_COLORS.length] },
        })),
        barWidth: 20,
      }],
    });
    ...
    const handleResize = () => {
      revenueChart.resize(); sourceChart.resize(); statusChart.resize();
    };
    window.addEventListener('resize', handleResize);
    return () => {
      window.removeEventListener('resize', handleResize);
      revenueChart.dispose(); sourceChart.dispose(); statusChart.dispose();
    };
  }, [customers, orders]);
  ...
  const addCustomer = () => {
    ...
    setCustomers((prev) => [{
      ...
      source: newCustomer.source, tags: newCustomer.tags.split(/[,，]/).filter(Boolean),
      totalSpent: 0, orderCount: 0, lastContact: new Date().toISOString().slice(0, 10),
    }, ...prev]);
    setNewCustomer({ name: '', phone: '', source: '小红书', tags: '' });
    toast.success('客户已添加');
  };
  ...
  const addFaq = () => {
    ...
    setFaqs((prev) => [{ id: generateId(), ...newFaq }, ...prev]);
    setNewFaq({ question: '', answer: '', category: '通用' });
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <Card className="p-8 text-center">
          <p className="text-lg font-medium">摄影师工作台</p>
          <p className="mt-2 text-sm text-muted-foreground">请在右上角切换到「摄影师」身份查看</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold">摄影师工作台</h1>
        <p className="text-muted-foreground">业务数据概览与管理</p>
      </div>
      ...
      <Tabs defaultValue="orders">
        <TabsList>
          ...
          <TabsTrigger value="faqs">FAQ 管理</TabsTrigger>
        </TabsList>
          ...
          <Card><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm">
            ...
            <tbody>{orders.map((o) => (
              <tr key={o.id} className="border-b last:border-0">
                ...
                <td className="p-3">{o.shootDate}</td>
                <td className="p-3"><Badge className={ORDER_STATUS_MAP[o.status].color}>{ORDER_STATUS_MAP[o.status].label}</Badge></td>
                <td className="p-3"><Select value={o.status} onValueChange={(v) => updateOrderStatus(o.id, v as OrderStatus)}><SelectTrigger className="h-8 w-28 text-xs"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(ORDER_STATUS_MAP).map(([k, v]) => <SelectItem key={k} value={k}>{v.label}</SelectItem>)}</SelectContent></Select></td>
              </tr>
            ))}</tbody>
